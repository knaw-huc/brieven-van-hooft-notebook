...
def __(mo, store):
    ...
    chosen_dataset = mo.ui.dropdown(options=available_datasets, value="gustave-pos", label="Dataset:")

    mo.md(f"""
    ### Exploring keys and data

    You can choose a dataset to explore possible keys and values: 
...
def __(chosen_dataset, mo, store):
    ...
    chosen_key = mo.ui.dropdown(options=available_keys,label="Key:")
...
@app.cell
def __(chosen_key, dataset, polars):
    key = dataset.key(chosen_key.value)
    vocab_dataframe = polars.DataFrame(
        data=((str(x), x.annotations_len()) for x in key.data()),
